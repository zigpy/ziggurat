...
impl<P: RadioPhy, R: Runtime> ZigbeeStack<P, R> {
    ...
    /// Start the Zigbee 3.0 trust-center link-key update once a freshly joined
    /// device announces itself. Most devices request this update themselves, but
    /// some BDB implementations wait for the trust center to initiate it and leave
    /// the network when their key-establishment timeout expires.
    pub(super) fn initiate_trust_center_link_key_update(
        &self,
        destination: Nwk,
        device_ieee: Eui64,
    ) {
        let (current_link_key, new_link_key) = {
            let mut core = self.core();

            if core.aib.aps_security.has_unique_link_key(device_ieee)
                || core.aib.aps_security.link_key_update_pending(device_ieee)
            {
                return;
            }

            let current_link_key = core.aib.aps_security.device_link_key(device_ieee);
            let new_link_key = core
                .aib
                .aps_security
                .issue_device_key(device_ieee, Key(crate::rng::random_array()));

            (current_link_key, new_link_key)
        };

        tracing::info!("Initiating trust center link key update for {device_ieee:?}");
            ...
            command: ApsCommandFrameCommand::TransportKey(ApsTransportKeyCommandFrame {
                ...
                key_descriptor: ApsTransportKeyDescriptor::TrustCenterLinkKey(
                    ApsTrustCenterLinkKeyDescriptor {
                        key: new_link_key,
                        destination_address: device_ieee,
                        source_address: self.state.ieee_address,
                    },
                ),
            }),
        ...
        self.send_secured_aps_payload(destination, encrypted_command.to_bytes())
            .map_err(|err| {
                tracing::warn!(
                    "Failed to initiate trust center link key update for {device_ieee:?}: {err}"
                );
            })
            .ok();
    }
